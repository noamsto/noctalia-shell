...
Scope {
  ...
  function startFingerprintAuth() {
    Logger.i("LockContext", "startFingerprintAuth called - fingerprintMode:", fingerprintMode,
             "FingerprintService.ready:", FingerprintService.ready,
             "FingerprintService.available:", FingerprintService.available,
             "pamStarted:", pamStarted, "unlockInProgress:", unlockInProgress,
             "currentText:", currentText !== "" ? "[has text: '" + currentText + "']" : "[empty]");
  ...
  function tryUnlock(fromEnterPress) {
    ...
    Logger.i("LockContext", "Starting PAM authentication:",
             "user:", pam.user,
             "configDirectory:", pamConfigDirectory,
             "config:", pamConfig,
             "fullPath:", pamConfigDirectory + "/" + pamConfig,
             "fingerprintMode:", fingerprintMode,
             "usePasswordOnly:", usePasswordOnly,
             "NOCTALIA_PAM_CONFIG env:", Quickshell.env("NOCTALIA_PAM_CONFIG") || "[not set]");
    pam.start();
  ...
  PamContext {
    ...
    onCompleted: result => {
                   var resultName = result === PamResult.Success ? "Success" :
                                    result === PamResult.AuthError ? "AuthError" :
                                    result === PamResult.CredentialsUnavailable ? "CredentialsUnavailable" :
                                    result === PamResult.AccountExpired ? "AccountExpired" :
                                    result === PamResult.MaxTries ? "MaxTries" :
                                    result === PamResult.PermissionDenied ? "PermissionDenied" :
                                    "Unknown(" + result + ")";
                   Logger.i("LockContext", "PAM completed - result:", result, "(" + resultName + ")",
                            "fingerprintMode:", root.fingerprintMode,
                            "usePasswordOnly:", root.usePasswordOnly,
                            "abortInProgress:", root.abortInProgress);
